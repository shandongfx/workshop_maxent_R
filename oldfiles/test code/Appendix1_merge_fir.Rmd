...
```{r setup knit}
library("knitr")
knitr::opts_knit$set(root.dir = 'E:/RWorkDir/MaxentInR')
...
#####Thread 4
```{r maxent,message=FALSE, eval=FALSE}
...
In our example, we used bioclimatic variables (downloaded from worldclim.org) as input environmental layers. We stacked our environmental layers so that they can be processed simultaneously.  

#####Thread 5 
```{r load rasters, message=FALSE}
...
clim_list <- list.files("../bioclim/",pattern=".bil$",full.names = T)
# the '..' in the above code is telling R to look above the folder where the .rmd file is located
...
# We provide an if/else statement that checks for occurrence data that has already been downloaded.
...
# if you want to View the first few lines of the occurrence dataset
head( occ_raw )
```
...
#####Thread 7
```{r clean data1}
# remove unsuitable occurences, where either the latitude or longitude is missing
...
Up to this point we have been working with a data frame, but it has no spatial relationship with environmental layers. So we need to make the data spatial. Once our data is spatial we can use the *plot* function to see the occurrence data and allow us to check for data points that appear to be erroneous. We can use the first layer of the bioclim data as a reference to spot erroneous occurence points. 
...
plot(occ_unique,add=TRUE) # plot the oc_unique on the above layer
...
#####Thread 10
```{r clean data4, eval=FALSE}
# thin occ data (keep one occurence point per cell)
cells <- cellFromXY(clim[[1]],occ_unique)
dups <- duplicated(cells)
occ_final <- occ_unique[!dups,]
cat(nrow(occ_unique)-nrow(occ_final), "records are removed")

# plot the first climatic layer 
plot(clim[[1]]) 
# you can replace [[1]] with any nth number of the layer of interest from the raster stack
...
plot(clim[[1]]) 
# the following code adds the occurrence data
plot(occ_final,add=T,col="red") 
# the following code adds the buffer polygon
plot(occ_buff,add=T,col="blue") 
```   

With a defined study area and the environmental layers stacked, we then cliped the layers to the extent of our study area. However, for ease of processing, we do this in two steps. First we create a coarse rectangular shaped study area around the study area to reduce the size of environmental data and then extract by *mask* using the buffer we created to more accurately clip environmental layers. This approach could be faster than directly masking. We save the cropped environmental layers as .asc (ascii files) to be used as inputs for Maxent.
...
dir.create("../data/studyarea")# to create a folder 'studyarea' where the new raster files will be stored
...
We randomly selected 50% of the occurrence data for model training and left the remaining for model testing. To make our experiment reproducable (i.e., select the same set of points), we used a static seed via *set.seed(1)* function.
...
The data input can either be spatial or tabular. In our example, we use the tabular format, which can be potentially more flexible. We extract environmental conditions for backround, training, and testing points in a dataframe format. 
...
Maxent reads a "1" as presence and a "0" as pseudo-absence. Thus, we need to assign a "1" to the training environmental conditions and a "0" for the background. We created a set of rows with the same number as the training and testing data, and put the value of "1" for each cell and a "0" for background. We combined the "1"s and "0"s into a vector that was added to the dataframe containing the environmental conditions associated with the testing and background conditions.
...
mod3 <- maxent(x=pder[c("bio1","bio11")], 
              ...
              args=prepPara(userfeatures="LQ",
                            betamultiplier=1,
                            projectionlayers="E:/RWorkDir/MaxentInR/data/studyarea") ) 
...
mod3 <- maxent(x=pder[c("bio1","bio11")], 
              ...
              args=prepPara(userfeatures="LQ",
                            betamultiplier=1,
                            projectionlayers="E:/RWorkDir/MaxentInR/bioclim") ) 
# plot the map
